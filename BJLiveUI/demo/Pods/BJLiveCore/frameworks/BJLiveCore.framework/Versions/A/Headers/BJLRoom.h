...
/**
 设置客户专属子域名，需要在进教室之前设置
 #param exclusiveSubdomain 客户专属子域名
 */
+ (void)setExclusiveSubdomain:(nullable NSString *)exclusiveSubdomain;
...
/** 部署环境(内部使用)，需要在进教室之前设置
 设置 `deployType` 将导致 `exclusiveSubdomain` 被重置 */
