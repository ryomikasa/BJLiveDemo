...
// 播放音视频
- (int) gsx_rtc_engine_play_media_start:(void *)in_pVoid :( const char *)in_pUrl :( int) in_nMediaType :( int) in_nRemoteId :( const char *)in_pPublishIp :( int) in_nPublishPort DEPRECATED_MSG_ATTRIBUTE("Use: -gsx_rtc_engine_play_media_start:::::::::");;
// 播放音视频 offset_index: 0:原始流 1:双摄像头（第二摄像头） 2:混音 3:低清流（第二路流）
- (int) gsx_rtc_engine_play_media_start:(void *)in_pVoid :( const char *)in_pUrl :( int) in_nMediaType :( int) in_nRemoteId :( const char *)in_pPublishIp :( int) in_nPublishPort :(int)in_nOffsetIndex;
