...
- (instancetype)init {
    self = [super init];
    if (self) {
        // !!!: only set for BJL-AFImageDownloader, but not AFImageDownloader
        NSURLSessionConfiguration *configuration = [BJLAFImageDownloader defaultURLSessionConfiguration];
        configuration.timeoutIntervalForRequest = BJLWebImageTimeoutInterval;
        configuration.timeoutIntervalForResource = MAX(60.0, BJLWebImageTimeoutInterval * 4);
        BJLAFImageDownloader *downloader = [[BJLAFImageDownloader alloc] initWithSessionConfiguration:configuration];
        [UIImageView bjlaf_setSharedImageDownloader:downloader];
        [UIButton bjlaf_setSharedImageDownloader:downloader];
    }
    return self;
}
