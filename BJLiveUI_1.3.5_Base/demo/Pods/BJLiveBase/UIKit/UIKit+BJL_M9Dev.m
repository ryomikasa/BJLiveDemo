...
- (nullable UIResponder *)bjl_findResponderWithBlock:(BOOL (NS_NOESCAPE ^)(UIResponder *responder, BOOL *stop))findingBlock {
    BOOL stop = NO;
...
- (void)bjl_eachView:(__kindof UIView *)view
            ...
            callback:(BOOL (NS_NOESCAPE ^)(UIView *subview, NSInteger depth))callback {
...
- (void)bjl_addChildViewController:(UIViewController *)childViewController addSubview:(void (NS_NOESCAPE ^)(UIView *parentView, UIView *childView))addSubview {
    if (!childViewController || !childViewController.view) return;
    [self addChildViewController:childViewController]; // 1
...
@dynamic bjl_topViewController;
- (nullable UIViewController *)bjl_topViewController {
    return [self bjl_topViewControllerExceptAlertController:NO];
}

- (nullable UIViewController *)bjl_topViewControllerExceptAlertController:(BOOL)exceptAlertController {
    ...
    while (topViewController.presentedViewController
           && (!exceptAlertController || ![topViewController.presentedViewController isKindOfClass:[UIAlertController class]])) {
...
+ (nullable UIViewController *)bjl_topViewController {
    return [self bjl_rootViewController].bjl_topViewController;
}

+ (nullable UIViewController *)bjl_topViewControllerExceptAlertController:(BOOL)exceptAlertController {
    return [[self bjl_rootViewController] bjl_topViewControllerExceptAlertController:exceptAlertController];
}
...
- (UIInterfaceOrientationMask)navigationControllerSupportedInterfaceOrientations:(UINavigationController *)navigationController {
    UIViewController *topViewController = [navigationController bjl_topViewControllerExceptAlertController:YES];
    return (topViewController
...
@implementation UITabBarController (BJL_M9Dev)
...
- (UIInterfaceOrientationMask)tabBarControllerSupportedInterfaceOrientations:(UITabBarController *)tabBarController {
    UIViewController *topViewController = [tabBarController bjl_topViewControllerExceptAlertController:YES];
    return (topViewController
            ? topViewController.supportedInterfaceOrientations
            : (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone
               ? UIInterfaceOrientationMaskAllButUpsideDown
