...
- (float)bjl_floatValue {
    return ([self respondsToSelector:@selector(floatValue)]
            ? [(id)self floatValue] : 0.0);
}

- (double)bjl_doubleValue {
    return ([self respondsToSelector:@selector(doubleValue)]
            ? [(id)self doubleValue] : 0.0);
}

- (long long)bjl_longLongValue {
    return ([self respondsToSelector:@selector(longLongValue)]
            ? [(id)self longLongValue] : 0LL);
}

- (unsigned long long)bjl_unsignedLongLongValue {
    return ([self respondsToSelector:@selector(unsignedLongLongValue)]
            ? [(id)self unsignedLongLongValue] : 0LLU);
}

- (BOOL)bjl_boolValue {
    return ([self respondsToSelector:@selector(boolValue)]
            ? [(id)self boolValue] : NO);
}

- (NSInteger)bjl_integerValue {
    return ([self respondsToSelector:@selector(integerValue)]
            ? [(id)self integerValue] : 0);
}

- (NSInteger)bjl_integerValueOrNotFound {
    return ([self respondsToSelector:@selector(integerValue)]
            ? [(id)self integerValue] : NSNotFound);
}

- (NSUInteger)bjl_unsignedIntegerValue {
    return ([self respondsToSelector:@selector(unsignedIntegerValue)]
            ? [(id)self unsignedIntegerValue] : 0);
}

- (NSUInteger)bjl_unsignedIntegerValueOrNotFound {
    return ([self respondsToSelector:@selector(unsignedIntegerValue)]
            ? [(id)self unsignedIntegerValue] : NSNotFound);
}
...
- (nullable id)bjl_objectForKey:(id)aKey callback:(_Nullable id (NS_NOESCAPE ^)(id object))callback {
    id object = [self objectForKey:aKey];
...
- (void)bjl_setObjectOrNil:(nullable id)anObject forKeyOrNil:(nullable id<NSCopying>)aKey {
    if (!aKey) {
        return;
    }
    if (!anObject) {
...
- (void)bjl_setObjectOrNil:(nullable id)anObject forKey:(nullable id<NSCopying>)aKey {
    [self bjl_setObjectOrNil:anObject forKeyOrNil:aKey];
}
...
+ (nullable instancetype)URLWithStringOrNil:(nullable NSString *)URLString {
    return URLString ? [self URLWithString:URLString] : nil;
}
